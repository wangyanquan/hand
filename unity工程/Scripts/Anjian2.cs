...
public class Anjian2 : MonoBehaviour {
    AudioSource myaudio;              //按键音乐
    Animator myani;                    //按键动画
    Blue myblue;                      //蓝牙组件
    void Awake()
    {
        myaudio = GameObject.Find("2").GetComponent<AudioSource>();
    ...
    void Update()
    {
        ...
        gameObject.transform.parent.position = new Vector3(-5.4f, 4, -3.471f);   //每次固定一下按钮，以免按键晃动
    }
    void OnCollisionEnter(Collision x)
    {
        ...
        else if (x.collider.name == "1_index_2" && myblue.sum[1] > 105)         //以下为检测到不同手指的情况
        {
            myaudio.Play();
            {
                myani.SetBool("down", true);
