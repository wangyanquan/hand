...
public class Anjian5 : MonoBehaviour {
    AudioSource myaudio;             //按键音乐
    Animator myani;                  //按键动画
    Blue myblue;                     //蓝牙组件
    void Awake()
    {
        myaudio = GameObject.Find("5").GetComponent<AudioSource>();
    ...
    void Update()
    {
        ...
        gameObject.transform.parent.position = new Vector3(-3.67f, 4, -3.471f);    //每次固定一下按钮，以免按键晃动
    }
