...
public class Anjian7 : MonoBehaviour {
    ...
    void Awake()
    {
        myaudio = GameObject.Find("7").GetComponent<AudioSource>();
    ...
    void Update()
    {
        ...
        gameObject.transform.parent.position = new Vector3(-2.52f, 4, -3.471f);      //每次固定一下按钮，以免按键晃动
    }
    void OnCollisionEnter(Collision x)
    {
        ...
        else if (x.collider.name == "1_index_2" && myblue.sum[1] > 105)            //以下为检测到不同手指的情况时时
        {
            myaudio.Play();
            {
                myani.SetBool("down", true);
